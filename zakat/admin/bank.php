    ...
    <section class="section dashboard">
      <div class="col-12">
        <div class="card overflow-auto">
          <div class="card-body">
            <h5 class="c-title">SENARAI BANK</h5>
            ...
            <table class="table table-borderless datatable1" id="table">
              ...
              <tbody>
              <?php
                $query_bank = $conn->query("SELECT * FROM bank");
                $i = 1;
                while($row = mysqli_fetch_assoc($query_bank)){
                  ?>
                <tr>
                  <th><?= $i ?></th>
                  <td><?= $row['bank_name'] ?></td>
                  <td class="text-end">
                    <a class="btn btn-danger btn-sm rounded-pill" href="<?= basename($_SERVER['PHP_SELF']).'?delete='.$row['bank_id'] ?>">Hapus</a>
                  </td>
                </tr>
            ...
            <form method="POST">
            <div class="row">
                  <label class="col-sm-3 col-form-label"><strong>Tambah bank baharu :</strong></label>
                  <div class="col-sm-2 mb-1">
                  <input class="form-control my-auto" type="text" name="bank_name" value="" required>
                  </div>
                  <div class="col-sm-2 my-1">
                  <button class="btn btn-primary mt-auto" type="submit" name="add_bank">Tambah</button>
                  </div>
                </div>
            </form>
          </div>
