    ...
    <section class="section dashboard">
      <div class="col-12">
        <div class="card overflow-auto">
          <div class="card-body">
            <h5 class="c-title">SENARAI TAJAAN</h5>
            ...
            <table class="table table-borderless datatable1" id="table">
              ...
              <tbody>
              <?php
                $query_taja = $conn->query("SELECT * FROM tajaan");
                $i = 1;
                while($row = mysqli_fetch_assoc($query_taja)){
                  ?>
                <tr>
                  <th><?= $i ?></th>
                  <td><?= $row['taja_name'] ?></td>
                  <td class="text-end">
                    <a class="btn btn-danger btn-sm rounded-pill" href="<?= basename($_SERVER['PHP_SELF']).'?delete='.$row['taja_id'] ?>">Hapus</a>
                  </td>
                </tr>
            ...
            <form method="POST">
            <div class="row">
                  <label class="col-sm-3 col-form-label"><strong>Tambah tajaan baharu :</strong></label>
                  <div class="col-sm-2 mb-1">
                  <input class="form-control mt-auto" type="text" name="taja_name" value="" required>
                  </div>
                  <div class="col-sm-2 my-1">
                  <button class="btn btn-primary mt-auto" type="submit" name="add_taja">Tambah</button>
                  </div>
                </div>
            </form>
          </div>
