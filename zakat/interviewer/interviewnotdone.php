...
if(isset($_GET['filter3'])){
  ...
  LEFT JOIN status ON interview_status = status_id
  WHERE interview_interviewer = '$inter_id' AND interview_status = 6 AND interview_sesi = '$filter'");
}elseif($_GET['filter1']){
  $filter = $_GET['filter1'];
  $interview = $conn->query("SELECT * FROM interview
  LEFT JOIN apply ON apply_id = interview_apply
  ...
  LEFT JOIN sesi ON sesi_id = apply_sesi
  LEFT JOIN status ON interview_status = status_id
  WHERE interview_interviewer = '$inter_id' AND interview_status = 6 AND apply_sem = '$filter'");
}else{
  $interview = $conn->query("SELECT * FROM interview
  LEFT JOIN apply ON apply_id = interview_apply
  LEFT JOIN semester ON sem_id = apply_sem
    ...
    <section class="section dashboard">
      <div class="col-12">
        <div class="card overflow-auto">
          <div class="card-body">
            <h5 class="c-title">Senarai Temuduga Selesai</h5>
            ...
            <a class="filter-title" href="" data-bs-toggle="dropdown">Pilihan Semester <i class="bi bi-chevron-down ms-auto"></i></a>
            <ul class="dropdown-menu dropdown-menu-arrow">
              <a href="<?= basename($_SERVER['PHP_SELF']); ?>"><li class="dropdown-header text-start">Papar Semua</li></a>
              <?php
              $filter_query1 = $conn->query("SELECT * FROM semester");
              while($row = mysqli_fetch_assoc($filter_query1)){
              ?>
              <a href="<?= basename($_SERVER['PHP_SELF']).'?filter1='.$row['sem_id']; ?>"><li class="dropdown-item"><?= $row['sem_name'] ?></li></a>
              <?php } ?>
            </ul>
            ...
            <table class="table datatable1" id="table">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td><?= $row['sem_name'] ?></td>
                  <td><strong class="<?= $row['status_id'] == 8 ? 'text-success':'text-primary'; ?>"><?= $row['status_name'] ?></strong></td>
                </tr>
