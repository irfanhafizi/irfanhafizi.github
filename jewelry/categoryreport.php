...
<div class="content">
    ...
    <input type="text" id="datepicker" class="form-control"name="filter2" onchange="form.submit()" autocomplete="off" placeholder="Select Month">
...
if($filter2){
    ...
    WHERE c_datetime LIKE '%$filter2%' AND shop = '$shop'");}

else if($filter4){$sql = mysqli_query($connection, "SELECT * FROM category
WHERE c_user_id = '$filter4' AND shop = '$shop' ");}
...
{ 
    ...
    { 
        ...
        echo '<tr class="report"><td>'.$no.'</td><td>'.$row['c_datetime'].'</td><td>'.$row['category'].'</td></tr>';
